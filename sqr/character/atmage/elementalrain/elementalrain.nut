	...
	else if (subState == SUB_STATE_ELEMENTAL_RAIN_FIRE)
	{
		if (obj.sq_IsMyControlObject())
		{
			...
			else if (sq_IsKeyDown(OPTION_HOTKEY_SKILL, ENUM_SUBKEY_TYPE_ALL))
			{
				obj.sq_IntVectClear();
				obj.sq_IntVectPush(SUB_STATE_ELEMENTAL_RAIN_LAST);
				obj.sq_AddSetStatePacket(STATE_ELEMENTAL_RAIN, STATE_PRIORITY_USER, true);
			}
		}
	}
}
...
{	
	...
	else if (subState == SUB_STATE_ELEMENTAL_RAIN_CHARGE)
	{
		...
		//是否指定?屬性
		local isWater = sq_GetIntData(obj, SKILL_ELEMENTAL_RAIN, 7);
		...
		{		
			for (local i = 0; i < maxNumber; ++i)
			{
				...
				//設置小魔法球屬性
				local randElementalType = ENUM_ELEMENT_WATER;
				if (isWater == 0) 
				{
					randElementalType = sq_getRandom(ENUM_ELEMENT_FIRE, ENUM_ELEMENT_LIGHT);
				}
