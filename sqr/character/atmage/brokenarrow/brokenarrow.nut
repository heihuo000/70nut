	...
	if(substate == SUB_STATE_BROKENARROW_0 || substate == SUB_STATE_BROKENARROW_1)
	{
		...
		if(frmIndex >= 3)
		{
			if(obj.getVar("flag").get_vector(0) == 0)
			{
				...
				local fireV = 100; //飛行速度比率
				local fireT = 200; //飛行時間，影響飛行速度主參數
				...
				local shootNum = obj.sq_GetIntData(SKILL_BROKENARROW, 7) / 100; //額外?箭數量
				...
				if(obj.isMyControlObject())
				{
					...
					for(local i = 0;i <= shootNum; i++)
					{
						...
						if (shootNum % 2 == 0)
						{
							...
							{
								obj.sq_SendCreatePassiveObjectPacket(24250, 0, fireOffsetX, fireOffsetY + 1 - (i + 1) / 2 * 10, fireOffsetZ); // 화살
							}
							//下排列
							else
							{
								obj.sq_SendCreatePassiveObjectPacket(24250, 0, fireOffsetX, fireOffsetY + 1 + i / 2 * 10, fireOffsetZ); // 화살
							}
						}
						else
						{
							...
							if (i % 2 == 0)
							{
								obj.sq_SendCreatePassiveObjectPacket(24250, 0, fireOffsetX, fireOffsetY - 4 - i / 2 * 10, fireOffsetZ); // 화살
							}
							//下排列
							else
							{
								obj.sq_SendCreatePassiveObjectPacket(24250, 0, fireOffsetX, fireOffsetY + 6 + (i - 1) / 2 * 10, fireOffsetZ); // 화살
							}
						}
					}
				}
