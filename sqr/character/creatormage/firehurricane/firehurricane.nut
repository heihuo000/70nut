...
function checkCommandEnable_FireHurricane(obj)
{
	...
	{
		return obj.sq_IsCommandEnable(SKILL_FIREHURRICANE); // °áÅõÀå¿¡¼­´Â Æ¯Á¤½ºÅ³¸¸ Äµ½½ÀÌ °¡´ÉÇÕ´Ï´Ù. ÀÛ¾÷ÀÚ:Á¤Áø¼ö [2012.04.20] obj.sq_IsCommandEnable(SKILL_BROKENARROW);
	}
...
function onSetState_FireHurricane(obj,state,datas,isResetTimer)
{
	...
	if (substate == SUB_STATE_FIREHURRICANE_0)
	{
		...
		local multiHitTerm = obj.sq_GetIntData(SKILL_FIREHURRICANE, 0); // 0.´Ù´ÜÈ÷Æ® °£°Ý
...
function onEnterFrame_FireHurricane(obj, frameIndex)
{
	...
	if (frameIndex == 2)
	{
		...
		local skill_level = obj.sq_GetIntData(132, 2);
		//local movableLevel = obj.sq_GetIntData(132, 2);	// 2.üæýè øÎÔÖ á½Õþ
		if (skill_level >= 1)
		{
			// üæýè øÎÔÖ
			local xVelocity = 140;//obj.sq_GetIntData(SKILL_FIREHURRICANE_EX, 3); // 3.Xõí üæýè ×§í¹
			local yVelocity = 120;//obj.sq_GetIntData(SKILL_FIREHURRICANE_EX, 4); // 4.Yõí üæýè ×§í¹
	...
	{
		local skill_level = obj.sq_GetSkillLevel(SKILL_FIREHURRICANE);
		local attackTotalTime = obj.sq_GetLevelData(SKILL_FIREHURRICANE, SKL_LV_1, skill_level); // 1.Áö¼Ó½Ã°£
