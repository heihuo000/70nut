// 职业角色 - 代码编写说明
// 此部分主要对特定职业角色的相关代码进行编写和说明。
// 以下内容以盗贼（Thief）职业角色 - 技能按钮相关逻辑为例进行说明。
// 若不是盗贼职业的角色，相关逻辑会进行相应修改。
// 函数的主要调用对象是大师（Master）等级的角色，或者是主角色（MainCharacter）。
